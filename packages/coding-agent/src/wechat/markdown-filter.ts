 ...
 * unsupported markdown syntax on-the-fly for WeChat messages.
 *
 ...
 *
 * Ported from @tencent-weixin/openclaw-weixin/src/messaging/markdown-filter.ts
 */
export class StreamingMarkdownFilter {
	...
	#pumpInline(_eof: boolean): string {
		if (!this.#inl) return "";
	...
	static #extractTableRow(line: string): string {
...
/**
 * Convenience: filter a complete markdown string in one call.
 */
