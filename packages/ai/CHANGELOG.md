...
- Fixed OpenAI-completions provider missing required `x-opencode-*` headers and `User-Agent` for opencode-zen/opencode-go requests, causing device-level rate limiting to fall through to shared `public` quota. Headers are now injected with a per-request unique `x-opencode-request` ID, a process-lifetime `x-opencode-session`, and CLI-matching `User-Agent`.
